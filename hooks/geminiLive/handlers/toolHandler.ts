  ...
  const handleToolCall = async (fc: any) => {
                  ...
                  if (modifiers !== undefined && targetVariant) {
                    ...
                    try {
                      if (typeof ctx.addToCart === "function") {
                        ...
                        result = `ADDED:${item.name}. SCREEN_READY. MUST_ASK: "Anything else?" MULTI_ADD_SUMMARY_HINT:"If several items were added this turn, say 'Added ${ctx.cart?.length ?? 1} items' instead of listing them."`;
                      } else {
    ...
    // ---------------- showCart ----------------
      ...
      try {
        ...
        result = `OPENED_CART_DRAWER: Success. CartCount:${cart.length}. Total: £${total}. CART_SUMMARY:${summary}. REPLY_SHORT:"You have ${cart.length} item(s), total £${total}. ${entries.length ? "Items: " + entries.join(", ") : ""}${remaining ? " plus " + remaining + " more." : ""}"`;
      } catch (e) {
        ...
        if (!targetVariant) {
          ...
          try {
            ...
            if (hasModifiers) {
              ...
              result = `ADDED:${item.name} (${targetVariant.name}). SCREEN_READY. MUST_ASK: "Anything else?" MULTI_ADD_SUMMARY_HINT:"If several items were added this turn, say 'Added ${ctx.cart?.length ?? 1} items' instead of listing them."`;
            }
          } catch (e) {
